/** ---------------
 * BACKGROUNDS
 * */
const backgrounds = [
    new Background({
        title: "Wheat Field with Cypresses",
        author: "Vincent van Gogh",
        year: "1889",
        url: "img/bg/bg-10.jpg"
    }),
    new Background({
        title: "",
        author: "",
        year: "",
        url: "img/bg/bg-7.jpg"
    }),
    new Background({
        title: "",
        author: "",
        year: "",
        url: "img/bg/bg-3.jpg"
    }),
    new Background({
        title: "",
        author: "",
        year: "",
        url: "img/bg/bg-5.jpg"
    }),
    new Background({
        title: "A View of the Mountain Pass Called the Notch of the White Mountains (Crawford Notch)",
        author: "Thomas Cole",
        year: "1839",
        url: "img/bg/bg-2.jpg"
    }),
    new Background({
        title: "Still Life with Apples and a Pot of Primroses",
        author: "Paul Cézanne",
        year: "1890",
        url: "img/bg/bg-8.jpg"
    }),
    new Background({
        title: "The Repast of the Lion",
        author: "Henri Rousseau (le Douanier) ",
        year: "1907",
        url: "img/bg/bg-9.jpg"
    }),
    new Background({
        title: "Forest Interior with a Waterfall, Papigno",
        author: "André Giroux",
        year: "1825/1830",
        url: "img/bg/bg-11.jpg"
    }),
    new Background({
        title: "Mortlake Terrace",
        author: "Joseph Mallord William Turner",
        year: "1827",
        url: "img/bg/bg-12.jpg"
    }),
    new Background({
        title: "Mountain Stream in the Auvergne",
        author: "Théodore Rousseau",
        year: "1830",
        url: "img/bg/bg-13.jpg"
    }),
    new Background({
        title: "Northern Landscape, Spring",
        author: "Caspar David Friedrich",
        year: "1825",
        url: "img/bg/bg-14.jpg"
    }),
    new Background({
        title: "The Tomb of Caecilia Metella",
        author: "Léon-François-Antoine Fleury",
        year: "1830",
        url: "img/bg/bg-15.jpg"
    }),
    new Background({
        title: "Tornado in an American Forest",
        author: "Thomas Cole",
        year: "1831",
        url: "img/bg/bg-16.jpg"
    }),
    new Background({
        title: "View from Vaekero near Christiania",
        author: "Johan Christian Dahl",
        year: "1827",
        url: "img/bg/bg-17.jpg"
    }),
    new Background({
        title: "View of the Castello di San Giuliano, near Trapani, Sicily",
        author: "Louise-Joséphine Sarazin de Belmont",
        year: "1824/1826",
        url: "img/bg/bg-18.jpg"
    }),
    new Background({
        title: "Northeaster",
        author: "Winslow Homer",
        year: "1895; reworked by 1901",
        url: "img/bg/bg-19.jpg"
    }),
    new Background({
        title: "An Arctic Summer: Boring Through the Pack in Melville Bay",
        author: "William Bradford",
        year: "1871",
        url: "img/bg/bg-20.jpg"
    }),
    new Background({
        title: "Dune Landscape with Oak Tree",
        author: "Jacob van Ruisdael",
        year: "1650–55",
        url: "img/bg/bg-21.jpg"
    }),
    new Background({
        title: "Nymphéas (Bagatelle)",
        author: "Eugène Atget",
        year: "1925",
        url: "img/bg/bg-22.jpg"
    }),
    new Background({
        title: "",
        author: "",
        year: "",
        url: "img/bg/bg-23.jpg"
    }),
    new Background({
        title: "",
        author: "",
        year: "",
        url: "img/bg/bg-24.jpg"
    }),
    new Background({
        title: "",
        author: "",
        year: "",
        url: "img/bg/bg-25.jpg"
    })
];